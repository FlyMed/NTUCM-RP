...
///////////////////////////////////////////////////////////// Do not touch below /////////////////////////////////////////
	...
	$('.input-chk').focusout(function() {
		if (!$(this).val()){
			$(this).addClass('error').parent().find('mark').removeClass('valid').addClass('error');
			$('#notification_3').removeClass('hide');
		}
		else{
			$(this).removeClass('error').parent().find('mark').removeClass('error').addClass('valid');
			$('#notification_3').addClass('hide');
		}
			
	});

	$('#submit').click(function() {
		$("#message").slideUp(20,function() {
			$('#message').hide();

			// Kick in Validation
			$('#email').triggerHandler("focusout");
			$('#comments').triggerHandler("focusout");
			$('.input-chk').triggerHandler("focusout");
	...
	$('#contactform').submit(function(){
		...
		$.post(action, $('#contactform').serialize(),
			function(data){
				...
				if(data['msg'].match('success') != null) {
